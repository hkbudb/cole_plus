use cole_plus_async::async_level::AsyncLevel;
use siri_tree::in_memory_siri::InMemorySIRI;
use cole_plus::{run::{LevelRun, reconstruct_run_proof, RunFilterSize}, RunProofOrHash};
use siri_tree::{traits::SIRINodeIO, SIRIRangeProof};
...
pub struct InMemGroup {
    pub mem_mht: InMemorySIRI, // in-mem SIRI
...
impl InMemGroup {
    pub fn new(exp_fanout: usize, max_fanout: usize) -> Self {
        Self {
            mem_mht: InMemorySIRI::new(exp_fanout, max_fanout),
            thread_handle: None,
        }
    }
...
impl<'a> ColePlusSIRI<'a> {
    ...
    pub fn insert(&mut self, state: (AddrKey, u32, StateValue)) {
        ...
        siri_tree::insert(tree_ref, CompoundKey::new(addr_key, ver), value);
    ...
    pub fn search_latest_state_value(&mut self, addr_key: AddrKey) -> Option<(AddrKey, u32, StateValue)> {
        ...
        match siri_tree::search_with_upper_key(write_tree, upper_key) {
        ...
        match siri_tree::search_with_upper_key(merge_tree, upper_key) {
            Some((read_key, read_v)) => {
                if read_key.addr == addr_key {
                    // matches the addresses and should be the latest value since latest value should be in the upper levels
                    return Some((read_key.addr, read_key.version, read_v));
    ...
    pub fn search_with_proof(&mut self, addr_key: AddrKey, lb: u32, ub: u32) -> ColePlusSIRIProof {
        let mut proof = ColePlusSIRIProof::new();
        ...
        let write_index = self.get_write_in_mem_group_index();
        let (r, p) = siri_tree::get_range_proof(&mut self.in_mem_group[write_index].mem_mht, low_key, upper_key);
        if r.is_some() {
        ...
        let merge_index = self.get_merge_in_mem_group_index();
        let (r, p) = siri_tree::get_range_proof(&mut self.in_mem_group[merge_index].mem_mht, low_key, upper_key);
        if r.is_some() {
            // check if the left_most version is smaller than the low_version, it means all the digests of the rest of the runs should be added to the proof
            // there is no need to prove_range the run
            let left_most_result = r.as_ref().unwrap()[0].0;
        ...
        for level in &mut self.levels {
            let mut level_proof = ColePlusSIRILevelProof::new();
            // write group
            let write_index = level.get_write_group_index();
            for run in level.run_groups[write_index].run_vec.iter_mut().rev() {
...
impl<'a> Drop for ColePlusSIRI<'a> {
    fn drop(&mut self) {
...
pub struct ColePlusSIRIProof {
    pub in_mem_level: ColePlusSIRIInMemProof,
    pub disk_level: Vec<ColePlusSIRILevelProof>,
}

impl ColePlusSIRIProof {
    // initiate the cole-start's proof
    pub fn new() -> Self {
        Self {
            in_mem_level: ColePlusSIRIInMemProof::new(),
            disk_level: Vec::new(),
        }
    }
}
...
#[derive(Debug, Serialize, Deserialize, Clone)]
pub struct ColePlusSIRIInMemProof {
    pub write_group: (Option<Vec<(CompoundKey, StateValue)>>, SIRIRangeProof<CompoundKey, StateValue>),
    pub merge_group: (Option<Vec<(CompoundKey, StateValue)>>, SIRIRangeProof<CompoundKey, StateValue>),
}

impl ColePlusSIRIInMemProof {
    pub fn new() -> Self {
        Self {
            write_group: (None, SIRIRangeProof::default()),
            merge_group: (None, SIRIRangeProof::default()),
        }
    }

    pub fn set_write_group(&mut self, r: Option<Vec<(CompoundKey, StateValue)>>, p: SIRIRangeProof<CompoundKey, StateValue>) {
        self.write_group = (r, p);
    }

    pub fn set_merge_group(&mut self, r: Option<Vec<(CompoundKey, StateValue)>>, p: SIRIRangeProof<CompoundKey, StateValue>) {
        self.merge_group = (r, p);
    }
}

#[derive(Debug, Serialize, Deserialize, Clone)]
pub struct ColePlusSIRILevelProof {
    pub write_group: Vec<(Option<Vec<VerObject>>, RunProofOrHash)>,
    pub merge_group: Vec<(Option<Vec<VerObject>>, RunProofOrHash)>,
}

impl ColePlusSIRILevelProof {
    pub fn new() -> Self {
        Self {
            write_group: Vec::new(),
            merge_group: Vec::new(),
...
pub fn verify_and_collect_result(addr_key: AddrKey, lb: u32, ub: u32, root_hash: H256, proof: &ColePlusSIRIProof, fanout: usize) -> (bool, Option<Vec<VerObject>>) {
    ...
    let h = siri_tree::reconstruct_range_proof(low_key, upper_key, write_group_result, write_group_proof);
    level_roots.push(h);
    ...
    let h = siri_tree::reconstruct_range_proof(low_key, upper_key, merge_group_result, merge_group_proof);
    level_roots.push(h);
