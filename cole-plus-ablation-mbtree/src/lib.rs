...
use cole_plus::{in_memory_mbtree::InMemoryMBTree, run::{LevelRun, RunFilterSize}};
use merkle_btree_storage::traits::BPlusTreeNodeIO;

use std::thread::JoinHandle;
use utils::{cacher::CacheManager, config::Configs, pager::{cdc_mht::CDCTreeReader, state_pager::{StateIterator, StatePageReader}, upper_mht::UpperMHTReader}, types::{compute_concatenate_hash, AddrKey, CompoundKey, StateValue}, MemCost, OpenOptions, Read, Write};
...
pub struct ColePlusAblationMBTree<'a> {
...
impl<'a> ColePlusAblationMBTree<'a> {
    ...
    pub fn insert(&mut self, state: (AddrKey, u32, StateValue)) {
        ...
        merkle_btree_storage::insert(tree_ref, CompoundKey::new(addr_key, ver), value);
...
impl<'a> Drop for ColePlusAblationMBTree<'a> {
    fn drop(&mut self) {
