...
impl<'a> ColeStar<'a> {
    ...
    pub fn insert(&mut self, state: (CompoundKey, StateValue)) {
        ...
        merkle_btree_storage::insert(mb_tree_ref, compound_key, value);
    ...
    fn check_and_merge(&mut self) {
        ...
        while level_id < self.levels.len() {
            if self.levels[level_id].level_write_group_reach_capacity(&self.configs) {
                ...
                let handle = std::thread::spawn(move|| {
                        let inputs: Vec<StateIteratorOld> = merge_group_run_id_vec.into_iter().map(|merge_run_id| {
                        ...
                        state_reader.to_state_iter_old()
                    }).collect();
...
mod tests {
    ...
    fn test_insert_and_query_cole_star() {
        ...
        let size_ratio = 10;
        let configs = Configs::new(fanout, epsilon, dir_name.to_string(), base_state_num, size_ratio, false, false, false);
