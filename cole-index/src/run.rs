...
use utils::{cacher::CacheManagerOld, config::{compute_bitmap_size_in_bytes, Configs}, models::CompoundKeyModel, old_design_merge_sort::{in_memory_merge_old_design, merge_old_design}, pager::{model_pager::ModelPageReader, old_mht_pager::{reconstruct_old_mht_range_proof, HashPageReaderOld, OldMHTRangeProof}, old_state_pager::{InMemStateIteratorOld, StateIteratorOld, StatePageReaderOld}, PAGE_SIZE}, types::{bytes_hash, AddrKey, CompoundKey, StateValue}, OpenOptions, Read, Write};
...
impl LevelRun {
    ...
    pub fn construct_run_by_in_memory_merge(inputs: Vec<InMemStateIteratorOld>, run_id: u32, level_id: u32, dir_name: &str, epsilon: i64, fanout: usize, max_num_of_state: usize, level_num_of_run: usize, size_ratio: usize) -> Self {
        ...
        let (state_writer, model_writer, mht_writer, filter) = in_memory_merge_old_design(inputs, &state_file_name, &model_file_name, &mht_file_name, epsilon, fanout, filter);
    ...
    pub fn construct_run_by_merge(inputs: Vec<StateIteratorOld>, run_id: u32, level_id: u32, dir_name: &str, epsilon: i64, fanout: usize, max_num_of_state: usize, level_num_of_run: usize, size_ratio: usize) -> Self {
        ...
        // merge the input states and construct the new state file, model file, mht file and insert state's keys to the filter
        let (state_writer, model_writer, mht_writer, filter) = merge_old_design(inputs, &state_file_name, &model_file_name, &mht_file_name, epsilon, fanout, filter);
     ...
     */
    pub fn prove_range(&mut self, addr_key: AddrKey, low_version: u32, upper_version: u32, configs: &Configs, cache_manager: &mut CacheManagerOld) -> (Option<Vec<(CompoundKey, StateValue)>>, RunProof) {
        // init the proof
        ...
        // compute the boundary compound key
        let lower_key = CompoundKey::new(addr_key, low_version);
        ...
        if left_proof_pos != 0 && states[lower_index_inner].0 == lower_key {
        ...
        // generate mht file range_proof
        let fanout = configs.fanout;
        let range_proof = self.mht_reader.prove_mht_file_range(self.run_id, left_proof_pos, right_proof_pos, num_of_states, fanout, cache_manager);
    ...
    pub fn format_run(&mut self, cache_manager: &mut CacheManagerOld) {
        ...
        for page_id in 0..model_page_num {
            let v = self.model_reader.read_deser_page_at_old_version(self.run_id, page_id, cache_manager).v;
            models.extend(&v);
        }
...
impl RunProof {
    ...
    pub fn include_range_proof_and_filer_hash(&mut self, range_proof: OldMHTRangeProof, filter_hash: Option<H256>) {
    ...
    pub fn get_range_proof_ref(&self) -> Option<&OldMHTRangeProof> {
...
pub fn reconstruct_run_proof(addr_key: AddrKey, low_version: u32, upper_version: u32, results: &Option<Vec<(CompoundKey, StateValue)>>, proof: &RunProof, fanout: usize) -> (bool, H256) {
    if results.is_none() {
        ...
        let obj_hashes: Vec<H256> = objs.iter().map(|elem| {
            let mut bytes = vec![];
            bytes.extend(elem.0.to_bytes());
            bytes.extend(elem.1.to_bytes());
            bytes_hash(&bytes)
        }).collect();
        let reconstruct_merkle_root = reconstruct_old_mht_range_proof(range_proof, fanout, obj_hashes);
...
pub  fn verify_run_proof(addr_key: AddrKey, low_version: u32, upper_version: u32, results: &Option<Vec<(CompoundKey, StateValue)>>, proof: &RunProof, fanout: usize, root_hash: H256) -> bool {
    if results.is_none() {
        // there is no result, the filter must filter the addr_key and the range proof is empty
        let filter_ref = proof.get_filter_ref();
        let merkle_root = proof.get_merkle_hash();
...
mod tests {
    ...
    use utils::pager::old_state_pager::StatePageWriterOld;
    use utils::types::{CompoundKey, StateValue};
    use primitive_types::{H160, H256};
    use utils::config::Configs;

    #[test]
    fn generate_states_old_design() {
        let k: usize = 2;
        let n: usize = 10;
        ...
        for i in 0..k {
            ...
            state_vec.push((min_key, H256::default().into()));
            state_vec.push((max_key, H256::default().into()));
    ...
    fn test_hash_old_design() {
        ...
        state_vec.push(((CompoundKey::new(AddrKey::new(acc_addr.into(), state_addr.into()), 34)), StateValue(H256::from_low_u64_be(34))));
        state_vec.push(((CompoundKey::new(AddrKey::new(acc_addr.into(), state_addr.into()), 36)), StateValue(H256::from_low_u64_be(36))));
    ...
    // before test this function, run generate_states_old_design first
    #[test]
    fn test_run_construct_and_load() {
        ...
        for i in 0..k {
            ...
            let iter = StateIteratorOld::create(file);
            iters.push(iter);
        }
