pub mod traits;
pub mod nodes;
pub mod in_memory_siri;
use std::collections::VecDeque;

use primitive_types::H256;
use traits::SIRINodeIO;
use serde::{Serialize, Deserialize};
use utils::types::{bytes_hash, Digestible, LoadAddr, Num, Value};
use nodes::{SIRILeafNode, SIRIInternalNode, SIRINode};
use poly_rolling::{RollingHash, RollingResult, DEFAULT_HASH_LEVEL};
...
pub fn get_search_path_ids<K: Num + LoadAddr, V: Value>(tree: &impl SIRINodeIO<K, V>, root_id: u32, key: K) -> Option<Vec<SearchNodeId>> {
...
fn search_key<K: Num + LoadAddr, V: Value>(tree: &impl SIRINodeIO<K, V>, key: K) -> Option<(bool, Vec<SearchNodeId>)> {
...
pub fn search_without_proof<K: Num + LoadAddr, V: Value>(tree: &impl SIRINodeIO<K, V>, key: K) -> Option<(K, V)> {
...
pub fn search_with_upper_key<K: Num + LoadAddr, V: Value>(tree: &impl SIRINodeIO<K, V>, key: K) -> Option<(K, V)> {
    let mut node_id = tree.get_root_id();
    if let Some(n) = tree.load_node(node_id) {
        let mut node = n;
        let mut value = V::default();
...
pub struct SIRIRangeProof<K: Num + LoadAddr, V: Value> {
    pub levels: Vec<Vec<((usize, usize), SIRINode<K, V>)>>, // the first two usize store the start_idx and end_idx of the searched entries in the node
}

pub fn get_range_proof<K: Num + LoadAddr, V: Value>(tree: &impl SIRINodeIO<K, V>, lb: K, ub: K) -> (Option<Vec<(K, V)>>, SIRIRangeProof<K, V>) {
    // init a range proof
    let mut proof = SIRIRangeProof::default();
    if tree.get_key_num() == 0 {
        ...
        let mut queue = VecDeque::<SIRINode<K, V>>::new();
        ...
        let mut cur_level_proof = Vec::<((usize, usize), SIRINode<K, V>)>::new();
...
pub fn reconstruct_range_proof<K: Num + LoadAddr, V: Value>(lb: K, ub: K, result: &Option<Vec<(K, V)>>, proof: &SIRIRangeProof<K, V>) -> H256 {
    if result.is_none() && proof == &SIRIRangeProof::default() {
        return H256::default();
    } else {
...
fn check_leaf_node_cdc_and_fanout<K: Num, V: Value>(list: &Vec<(K, V)>, exp_fanout: usize, max_fanout: usize) -> Vec<i32> {
    ...
    let mut cdc_hash = RollingHash::new(exp_fanout, DEFAULT_HASH_LEVEL, max_fanout);
    ...
    for cur_index in 0..list_len {
        ...
        if let RollingResult::PatternFound | RollingResult::ReachCapacity = r {
...
pub fn insert<K: Num + LoadAddr, V: Value>(tree: &mut impl SIRINodeIO<K, V>, key: K, value: V) {
    ...
    match search_key(tree, key) {
        Some((key_exist, node_id_vec)) => {
            ...
            for i in 1..=cut_points_len - 1 {
                ...
                let mut new_node = SIRILeafNode::new(collection[start_idx..=end_idx].to_vec());
                ...
                tree.store_node(new_node_id, SIRINode::from_leaf(new_node));
            }
        ...
        None => {
            ...
            let leaf = SIRILeafNode::new(key_values);
            let node = SIRINode::from_leaf(leaf);
...
fn handle_insert_parent_nodes<K: Num + LoadAddr, V: Value>(tree: &mut impl SIRINodeIO<K, V>, entries: PushUpEntries<K>, node_id_vec: Vec<SearchNodeId>, collect_num_nodes: usize) {
    // get exp and max fanout
    let exp_fanout = tree.get_exp_fanout();
    let max_fanout = tree.get_max_fanout();
    if node_id_vec.is_empty() {
        if entries.entry_num() >= 2 {
            ...
            for i in 1..=cut_points_len - 1 {
                let start_idx = (cut_points[i-1] + 1) as usize;
                let end_idx = cut_points[i] as usize;
                let mut new_node = SIRIInternalNode::new(key_collection[start_idx..=end_idx].to_vec(), child_id_collection[start_idx..=end_idx].to_vec(), child_hash_collection[start_idx..=end_idx].to_vec());
                new_node.next = pointers[i+1];
                new_node.prev = pointers[i-1];
                let new_node_id = pointers[i];
                ...
                pushed_node_ids.push(new_node_id);
                pushed_node_hashes.push(new_node.to_digest());
                tree.store_node(new_node_id, SIRINode::from_internal(new_node));
            }
            let entries = PushUpEntries::new(pushed_last_keys, pushed_node_hashes, pushed_node_ids);
...
pub fn remove<K: Num + LoadAddr, V: Value>(tree: &mut impl SIRINodeIO<K, V>, key: K) {
    // get exp and max fanout
    let exp_fanout = tree.get_exp_fanout();
    let max_fanout = tree.get_max_fanout();
    match search_key(tree, key) {
...
fn handle_delete_parent_nodes<K: Num + LoadAddr, V: Value>(tree: &mut impl SIRINodeIO<K, V>, entries: PushUpEntries<K>, node_id_vec: Vec<SearchNodeId>, collect_num_nodes: usize) {
    // get exp and max fanout
    let exp_fanout = tree.get_exp_fanout();
    let max_fanout = tree.get_max_fanout();
    if node_id_vec.is_empty() {
        ...
        if collect_num_nodes == 0 {
            ...
            if node.get_n() == 0 {
                ...
                tree.store_node(node_id, SIRINode::from_internal(node));
                let entries = PushUpEntries::new(pushed_last_keys, pushed_node_hashes, pushed_node_ids);
                handle_delete_parent_nodes(tree, entries, node_id_vec, 1);
            }
        } else {
...
pub fn get_left_most_leaf_id<K: Num + LoadAddr, V: Value>(tree: &impl SIRINodeIO<K, V>) -> u32 {
    // get root id
    let mut node_id = tree.get_root_id();
    // retrieve root node
...
pub fn get_left_path<K: Num + LoadAddr, V: Value>(tree: &impl SIRINodeIO<K, V>) -> Vec<u32> {
    // get root id
    let mut node_id = tree.get_root_id();
    // retrieve root node
    let mut node = tree.load_node(node_id).unwrap();
...
pub fn get_tree_height<K: Num + LoadAddr, V: Value>(tree: &impl SIRINodeIO<K, V>) -> usize {
    let mut height = 0;
    // get root id
    let mut node_id = tree.get_root_id();
    height += 1;
...
pub fn check_prev_next_pointers<K: Num + LoadAddr, V: Value>(tree: &impl SIRINodeIO<K, V>) -> bool {
    let mut flag = true;
...
pub fn check_pointers<K: Num + LoadAddr, V: Value>(tree: &impl SIRINodeIO<K, V>) -> bool {
    let mut flag = true;
...
pub fn print_tree<K: Num + LoadAddr, V: Value>(tree: &impl SIRINodeIO<K, V>) -> usize {
...
mod tests {
    ...
    pub struct TestPOSTree {
        ...
        pub counter: u32,
        pub key_num: u32,
        pub nodes: BTreeMap<u32, SIRINode<IKey, IValue>>,
    ...
    impl SIRINodeIO<IKey, IValue> for TestPOSTree {
        fn load_node(&self, node_id: u32) -> Option<SIRINode<IKey, IValue>> {
        ...
        fn store_node(&mut self, node_id: u32, node: SIRINode<IKey, IValue>) {
            self.nodes.insert(node_id, node);
        }
    
        fn store_nodes_batch(&mut self, nodes: BTreeMap::<u32, SIRINode<IKey, IValue>>) {
            self.nodes.extend(nodes);
        }
    
        fn remove_node(&mut self, node_id: u32) -> Option<SIRINode<IKey, IValue>> {
