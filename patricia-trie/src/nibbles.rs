...
use utils::types::{Digestible, AddrKey};
...
impl NibbleBuf {
    ...
    pub fn from_h256(h: &H256) -> Self {
        let s = to_hex(h.as_bytes(), false);
        Self::from_hex_str(&s)
    }

    pub fn from_h160_and_h256(h160: H160, h256: H256) ->Self {
        let mut bytes = h160.as_bytes().to_vec();
        bytes.extend(h256.as_bytes());
        let s = to_hex(&bytes, false);
        Self::from_hex_str(&s)
    }

    pub fn from_addr_key(addr_key: AddrKey) -> Self {
        Self::from_h160_and_h256(addr_key.addr.0, addr_key.state_key.0)
    }
...
pub fn to_hex(bytes: &[u8], skip_leading_zero: bool) -> String {
	let mut slice = vec![0u8; bytes.len() * 2];
...
mod tests {
    ...
    #[test]
    fn test_addr_key() {
        let addr_key = AddrKey::new(H160::random().into(), H256::random().into());
        println!("addr_key: {:?}", addr_key);
        let nibble_buf = NibbleBuf::from_addr_key(addr_key);
        println!("{:?}", nibble_buf);
    }
