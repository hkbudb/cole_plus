...
pub fn compound_key_to_integer(key: &CompoundKey) -> Integer {
    let mut bytes = key.addr.addr.to_bytes();
    bytes.extend(&key.addr.state_key.to_bytes());
    bytes.extend(key.version.to_be_bytes());
...
impl ModelGenerator {
    ...
    pub fn append(&mut self, key: &CompoundKey, pos: usize) -> bool {
        ...
        if &key_int == last_point_in_pgm && pos as i64 == self.pgm.last_y {
            // println!("duplicate, key: {:?}, pos: {:?}", key, pos);
            return true;
        }
    ...
    pub fn finalize_model(&mut self) -> CompoundKeyModel {
        ...
        self.pgm.reset();
        self.start = None;
        // println!("finalize start: {:?}, slope: {:?}, intercept: {:?}", start, slope, intercept);
        CompoundKeyModel {
            start,
            slope: slope.to_f64(),
    ...
    pub fn finalize_raw_model(&mut self) -> RawModel {
        let seg = self.pgm.get_segment();
        let start = self.start.unwrap().clone();
        let start_int = compound_key_to_integer(&start);
        let last_pos = seg.get_last_y();
...
impl CompoundKeyModel {
    ...
    pub fn predict(&self, key: &Integer) -> usize {
        ...
        // println!("predict start int: {:?}, slope: {}, intercept: {}", start_int, self.slope, self.intercept);
        let pos = ((key - start_int) * &Rational::from_f64(self.slope).unwrap()).complete() + &Rational::from_f64(self.intercept).unwrap();
        let mut pos_f64 = pos.to_f64();
        if (pos_f64 - pos_f64.round()).abs() < 1.0e-10 {
            pos_f64 = pos_f64.round();
        }
        // println!("after pos 64: {}", pos_f64);
        let pos_integer = pos_f64 as usize;
...
impl RawModel {
    pub fn get_slope_intercept(&self) -> (Rational, f64) {
        (self.slope.clone(), self.intercept)
    }
    ...
    pub fn predict(&self, key: &Integer) -> usize {
        let start_int = compound_key_to_integer(&self.start);
        let pos = ((key - start_int) * &self.slope).complete() + &Rational::from_f64(self.intercept).unwrap();
    ...
    pub fn to_bytes(&self) -> Vec<u8> {
        let mut total_bytes = self.start.to_bytes();
        total_bytes.extend(&bincode::serialize(&self.slope).unwrap());
        // total_bytes.extend(&self.slope.to_be_bytes());
        total_bytes.extend(&self.intercept.to_be_bytes());
        // total_bytes.extend(&bincode::serialize(&self.intercept).unwrap());
        total_bytes.extend(&self.last_index.to_be_bytes());
        return total_bytes;
    }
}
...
pub fn fetch_model_and_predict(final_models: &Vec<CompoundKeyModel>, key: &CompoundKey) -> usize {
    ...
    // println!("predict key: {:?}, key_integer: {:?}", key, key_integer);
    let mut index;
...
pub fn fetch_raw_model_and_predict(final_models: &Vec<RawModel>, key: &CompoundKey, verbose: bool) -> usize {
    let key_integer = compound_key_to_integer(&key);
    let mut index;
    let model_len = final_models.len();
    let mut l: i32 = 0;
...
mod tests {
    use rug::Integer;
    use rand::prelude::*;
    use crate::pager::state_pager::StatePageWriter;
    use crate::types::{AddrKey, Address, CompoundKey, StateKey, StateValue};
    use primitive_types::{H160, H256};
    use super::{fetch_model_and_predict, fetch_raw_model_and_predict, CompoundKeyModel, ModelGenerator, RawModel};
    ...
    fn test_compound_key() {
        ...
        let low = CompoundKey::new(keys[0].addr, 0);
        println!("low: {:?}", low);
    ...
    fn test_raw_model() {
        ...
        let mut model_vec: Vec<RawModel> = Vec::new();
        for state in &states {
            let model_key = state_page_writer.append(*state);
            if let Some(inner_model_key) = model_key {
                let r = model_generator.append(&inner_model_key, page_cnt);
                if r == false {
                    let model = model_generator.finalize_raw_model();
                    model_vec.push(model);
                    model_generator.append(&inner_model_key, page_cnt);
                }
        ...
        println!("page cnt: {}", page_cnt);
        if !model_generator.is_hull_empty() {
            let model = model_generator.finalize_raw_model();
            model_vec.push(model);
        }

        for (i, m) in model_vec.iter().enumerate() {
            // println!("i: {}, model: {:?}", i, m);
            println!("i: {}, model len: {:?}, model: {:?}", i, m.to_bytes().len(), m);
        }

        state_page_writer.flush();
        for state in &states {
            let search_key = CompoundKey::new(state.0, state.1);
            let pred_pos = fetch_raw_model_and_predict(&model_vec, &search_key, false);
            // let pred_pos = model_reader.get_pred_pos(run_id, &search_key, false, &mut cache_manager);
            ...
            if (true_pos as f64 - pred_pos as f64).abs() > 2.0 {
                ...
                fetch_raw_model_and_predict(&model_vec, &search_key, true);
                break;
            }
        }
    }
