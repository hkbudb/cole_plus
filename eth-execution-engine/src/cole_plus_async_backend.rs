...
use anyhow::Result;
use cole_plus_async::ColePlusAsync;
use utils::MemCost;

pub struct ColePlusAsyncBackend<'a> {
    pub nonce_map: BTreeMap<Address, Nonce>,
    pub code_map: BTreeMap<Address, Code>,
    pub states: ColePlusAsync<'a>,
    pub path: &'a str,
}

impl<'a> ColePlusAsyncBackend<'a> {
    pub fn new(configs: &'a Configs, path: &'a str) -> Self {
        Self {
            nonce_map: BTreeMap::new(), 
            code_map: BTreeMap::new(), 
            states: ColePlusAsync::new(configs),
            path,
        }
    }

    pub fn get_mut_total_tree(&self) -> &'a mut ColePlusAsync<'a> {
        unsafe {
            let const_ptr = &self.states as *const ColePlusAsync;
            let mut_ptr = UnsafeCell::new(const_ptr as *mut ColePlusAsync);
            &mut **mut_ptr.get()
        }
    }
}

impl<'a> Backend for ColePlusAsyncBackend<'a> {
...
impl<'a> BackendWriteTrait for ColePlusAsyncBackend<'a> {
...
mod tests {
    ...
    #[test]
    fn test_cole_plus_async_backend() {
        let fanout = 5;
        let dir_name = "cole_storage";
        if std::path::Path::new(dir_name).exists() {
            std::fs::remove_dir_all(dir_name).unwrap_or_default();
