...
use anyhow::Result;
use cole_plus_ablation_siri::ColePlusSIRI;
use utils::MemCost;

pub struct ColePlusSIRIBackend<'a> {
    pub nonce_map: BTreeMap<Address, Nonce>,
    pub code_map: BTreeMap<Address, Code>,
    pub states: ColePlusSIRI<'a>,
    pub path: &'a str,
}

impl<'a> ColePlusSIRIBackend<'a> {
    pub fn new(configs: &'a Configs, path: &'a str) -> Self {
        Self {
            nonce_map: BTreeMap::new(), 
            code_map: BTreeMap::new(), 
            states: ColePlusSIRI::new(configs),
            path,
        }
    }

    pub fn get_mut_total_tree(&self) -> &'a mut ColePlusSIRI<'a> {
        unsafe {
            let const_ptr = &self.states as *const ColePlusSIRI;
            let mut_ptr = UnsafeCell::new(const_ptr as *mut ColePlusSIRI);
            &mut **mut_ptr.get()
        }
    }
}

impl<'a> Backend for ColePlusSIRIBackend<'a> {
...
impl<'a> BackendWriteTrait for ColePlusSIRIBackend<'a> {
...
mod tests {
    ...
    #[test]
    fn test_cole_plus_ablation_backend_storage() {
        let fanout = 5;
        ...
        let mut backend = ColePlusSIRIBackend::new(&configs, dir_name);
