...
pub fn test_cole_plus_short_reorg(params: &ReOrgParams) -> Result<()> {
    ...
    let mut short_reorg_file = OpenOptions::new().create(true).read(true).write(true).truncate(true).open(format!("{}-short-reorg.json", base)).unwrap();
    ...
    let is_pruned = false;
    let configs = Configs::new(params.mht_fanout, params.epsilon as i64, db_path.to_string(), params.mem_size, params.size_ratio, is_pruned, test_in_mem_roll, false);
    ...
    backend.rewind_in_mememory_states(rewind_blocks as u32);
    let now = Utc::now();
    ...
    write!(short_reorg_file, "{}\n", ts_result_str).unwrap();

    short_reorg_file.flush().unwrap();
    return Ok(());
}